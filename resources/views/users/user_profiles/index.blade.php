    ...
    <div class="container">
        <div class="card">
            <div class="card-header d-flex flex-column flex-md-row align-items-md-center justify-content-between">
                ...
                <form action="{{ route('users.user_profiles.index', compact('user')) }}" method="GET" class="m-0 p-0">
    ...
    @foreach($userProfiles as $userProfile)
        <tr>
            ...
            <td data-label="Actions:" class="text-nowrap">
                                   <a href="{{route('users.user_profiles.show', compact('user', 'userProfile'))}}" type="button" class="btn btn-primary btn-sm me-1">@lang('Show')</a>
<div class="btn-group btn-group-sm">
    <button type="button" class="btn btn-light dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false"><i class="fa fa-cog"></i></button>
    <ul class="dropdown-menu">
        <li><a class="dropdown-item" href="{{route('users.user_profiles.edit', compact('user', 'userProfile'))}}">@lang('Edit')</a></li>
        <li>
            <form action="{{route('users.user_profiles.destroy', compact('user', 'userProfile'))}}" method="POST" style="display: inline;" class="m-0 p-0">
                @csrf
                @method('DELETE')
    ...
    @endforeach
            ...
            <div class="text-center my-2">
                <a href="{{ route('users.user_profiles.create', compact('user')) }}" class="btn btn-primary"><i class="fa fa-plus"></i> @lang('Create new User Profile')</a>
            </div>
        </div>
    </div>
@endsection
